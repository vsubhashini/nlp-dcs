      ...
      val dlog = new DatalogInterpreter(U) {
        override def processSpecialStatement(path:String, statement:StatementItem, name:String, args:List[ExprItem]) = name match {
          case "_parse" => args match {
            case ListItem(wordItems) :: ListItem(ansItems) :: rest =>
            //case ListItem(wordItems) :: trueExpr :: rest =>
              ...
              val answers = ansItems.map(toString)
              if (words.size <= maxSentenceLength) {
                //val ansPred = currWorldInt.executeQuerySafe(trueExpr)
  		//Utils.logs("trueExpr => %s | ansPredString => %s |Arity=> %s | Size=> %s", trueExpr, ansPred.name, ansPred.arity, ansPred.size)
                //val ex = new BaseExample(newId, words, U.currWorld, new Answer(currWorldInt.executeQuerySafe(trueExpr)), groupId)
                val ansPred = new ExplicitPredicate(1)
		ansItems.foreach { subitem =>
		  ansPred += Entity(currWorldInt.item2value(subitem))
		  //ansPred += Entity(currWorldInt.item2value(subitem.mkString().replaceAll("_"," ")))
		} 
                val ex = new BaseExample(newId, words, U.currWorld, new Answer(ansPred), groupId)
		//val ans = new Answer(currWorldInt.executeQuerySafe(trueExpr))
  		//Utils.logs(" Answer => %s ", ans)
                //ex.trueExpr = trueExpr
 		Utils.logs(" Question => %s | Answers => %s ", words.mkString(" "), answers.mkString(","))
                printEx(ex)
                add(ex)
              }
